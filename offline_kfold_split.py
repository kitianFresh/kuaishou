...
for i in range(kfold):
    ...
    kfold_user_item_train = user_item_train.append(kfold_user_item_val).drop_duplicates(keep=False).reset_index(
        drop=True)
    val_photo_ids = set(kfold_user_item_val['photo_id'].unique()) - set(kfold_user_item_train['photo_id'].unique())
    inter_train_val_photo_ids = set(kfold_user_item_val['photo_id'].unique()) & set(
        kfold_user_item_train['photo_id'].unique())
    ...
    print("train shape after validation remove intersection: (%d, %d)" % (
    kfold_user_item_train.shape[0], kfold_user_item_train.shape[1]))
    print("valid shape after validation remove intersection: (%d, %d)" % (
    kfold_user_item_val.shape[0], kfold_user_item_val.shape[1]))
