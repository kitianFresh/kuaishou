...
def train(data,cluster_nums,model_path):
    ...
    kmeans.fit(X_pqcode)
    pickle.dump(encoder,open(model_path + '_encoder.pkl','wb'))
    pickle.dump(kmeans,open(model_path + '_kmeans.pkl','wb'))
    clustered = kmeans.predict(X_pqcode)
    return clustered

def predict(data,model_path):
    encoder = pickle.load(open(model_path + '_encoder.pkl','rb'))
    kmeans = pickle.load(open(model_path + '_kmeans.pkl','rb'))
    X_pqcode = encoder.transform(data)
    clustered = kmeans.predict(X_pqcode)
    return clustered
